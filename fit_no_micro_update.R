...
for (s in subs) {
  ...
  ldc_file <- paste0('allpar/eta_unbound/no_micro_update/batch_',Nupdate_per_trial,'/ldcfit_',s,'.Rdata')
  ...
  if (model=="allpar") {
    par[par$sub==s&par$condition=="plus","a0"] <- ldc.results$optim$bestmem[5]
    par[par$sub==s&par$condition=="plus","b0"] <- ldc.results$optim$bestmem[6]
    par[par$sub==s&par$condition=="plus","eta"] <- ldc.results$optim$bestmem[8]
  }
  
  train[train$sub==s&train$condition=="minus",'cj_pred'] <-
    ldc.nn.fit.w(params=c(par[par$sub==s&par$condition=="minus","a0"],
                          par[par$sub==s&par$condition=="minus","b0"],1,
                          par[par$sub==s&par$condition=="minus","eta"]),
  ...
  train[train$sub==s&train$condition=="plus",'cj_pred'] <-
    ldc.nn.fit.w(params=c(par[par$sub==s&par$condition=="plus","a0"],
                          par[par$sub==s&par$condition=="plus","b0"],1,
                          par[par$sub==s&par$condition=="plus","eta"]),
...
jpeg(filename = "traces2.jpg",units = 'cm',width = 42,height = 30,res=300)
