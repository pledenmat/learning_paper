...
for (s in subs) {
  ...
  for (cond in conditions) {
    ...
    if (file.exists(ldc_file)) {
      ...
      optimal_params <- DEoptim(ldc.nn.fit.w,ddm_params=ddm_params,obs = temp_dat,
                                lower = c(0,-100,1,0), 
                                upper = c(50,100,1,1),
    ...
    par[par$sub==s&par$condition==cond,"a0"] <- ldc.results$optim$bestmem[1]
    par[par$sub==s&par$condition==cond,"b0"] <- ldc.results$optim$bestmem[2]
    par[par$sub==s&par$condition==cond,"eta"] <- ldc.results$optim$bestmem[4]
    par[par$sub==s&par$condition==cond,"cost_ldc"] <- ldc.results$optim$bestval
    
    results <-
      ldc.nn.fit.w(params=c(mean(par[par$sub==s&par$condition==cond,"a0"]),
                            mean(par[par$sub==s&par$condition==cond,"b0"]),1,
                            mean(par[par$sub==s&par$condition==cond,"eta"])),
                   ddm_params = ddm_params,
                   obs=temp_dat,returnFit = F,
                   Nupdate_per_trial=Nupdate_per_trial, binning = binning,
                   dt = dt, sigma = sigma)
    train[train$sub==s&train$condition==cond,'cj_pred'] <- results$pred
    
    par_trace[par_trace$sub==s&par_trace$condition==cond,"alpha"] <-
      c(results$trace[,1],
        rep(NA,ntrial-length(results$trace[seq(Nupdate_per_trial,nrow(results$trace),Nupdate_per_trial),1])))
    par_trace[par_trace$sub==s&par_trace$condition==cond,"beta"] <-
      c(results$trace[,2],
...
# anova(m)
# with(par,aggregate(a0,by=list(manip,condition),mean))
# 
# m <- lmer(data = par, b0 ~ condition*manip + (1|sub))
# anova(m)
# with(par,aggregate(b0,by=list(manip,condition),mean))
# 
# m <- lmer(data = par, eta ~ condition*manip + (1|sub))
# anova(m)
# with(par,aggregate(eta,by=list(manip,condition),mean))
# 
...
polygon(c(1:xlen,xlen:1),c(conf_plus_cor + conf_plus_cor_se,(conf_plus_cor - conf_plus_cor_se)[xlen:1]),
        border=F,col=rgb(213,94,0,51,maxColorValue = 255))
legend(cex=cex.legend,"bottomleft",legend = c(expression(paste("high ",alpha," feedback")), expression(paste("low ",alpha," feedback"))),
       col = c(VERMILLION,BLUE), bty = 'n', lty = c(1,1))
...
alpha_trace_plus <- cast(subset(alpha_trace,condition=="plus" & !(sub %in% par_higheta$sub)),sub~trial)
...
beta_trace_plus <- cast(subset(beta_trace,condition=="plus"& !(sub %in% par_higheta$sub)),sub~trial)
...
# Retrieve parameter traces -- NOW IN FIT RETRIEVAL -----------------------------------------------
ntrial <- 108
par_trace <- data.frame(trial=1:ntrial,sub=rep(subs,each=ntrial*2),
                        condition=rep(c("plus","minus"),each=ntrial),
                        alpha=NA,beta=NA)
for (s in subs) {
  ...
  totRTname='rt2';targetname='fb';accname='cor';beta_input=.1;error_type='mse'
  nbin <- 6
  sigma <- .1
  dt <- .001
  ...
  results <- train_model(x,w0,y,eta=mean(subset(par,sub==s&condition=="plus")$eta),
                         error_type1 = error_type1,trace=T,error_type2 = error_type2,
