...
simDat_alpha$cj_continuous <- simDat_alpha$cj
simDat_alpha$cj_bin <- as.numeric(cut(simDat_alpha$cj,breaks=seq(0,1,length.out=7),include.lowest = T))
simDat_alpha$cj <- simDat_alpha$cj_bin/6
simDat_alpha$cj <- simDat_alpha$cj - 1/12 # Try fixing bin values to center of their range
for (s in 1:Nsub) {
...
for (s in 1:Nsub) {
  ...
  ldc_file <- paste0('fit/alternating_fb/ldc_nn/recovery/binned_confidence/sim_alpha_learn/beta_model/ldcfit_',subs[s],'.Rdata')
  if (file.exists(ldc_file)) {
    load(ldc_file)
  }else{
    optimal_params <- DEoptim(ldc.nn.fit.w,ddm_params=ddm_params, 
...
for (s in 1:Nsub) {
  ...
  ldc_file <- paste0('fit/alternating_fb/ldc_nn/recovery/binned_confidence/sim_alpha_learn/alpha_model/ldcfit_',subs[s],'.Rdata')
  if (file.exists(ldc_file)) {
    load(ldc_file)
  }else{
    optimal_params <- DEoptim(ldc.nn.fit.w,ddm_params=ddm_params, 
...
for (s in 1:Nsub) {
  ...
  ldc_file <- paste0('fit/alternating_fb/ldc_nn/recovery/binned_confidence/sim_alpha_learn/no_learn/ldcfit_',subs[s],'.Rdata')
  if (file.exists(ldc_file)) {
    load(ldc_file)
  }else{
    optimal_params <- DEoptim(ldc.nn.fit.w,ddm_params=ddm_params, 
...
for (s in 1:Nsub) {
  ...
  ldc_file_beta_learn <- paste0('beta_model/ldcfit_',subs[s],'.Rdata')
  ...
  ldc_file_both_learn <- paste0('both_model/ldcfit_',subs[s],'.Rdata')
  ...
  ldc_file_no_learn <- paste0('no_model/ldcfit_',subs[s],'.Rdata')
  if (file.exists(ldc_file_no_learn)) {
    load(ldc_file_no_learn)
...
# Plot parameters for all models ------------------------------------------
...
ggplot(subset(par,!(model %in% c("alpha","no"))), aes(x = eta_b, colour = model, label = model)) +
  geom_density() +
  theme_bw()
ggplot(subset(par,model=="beta"), aes(x = eta_b, colour = model, label = model)) +
  geom_density() +
  theme_bw()
...
# Scatterplot generative/recovered parameters -----------------------------
gen_par_alpha_learn <- aggregate(cbind(a0,b0,eta_a) ~ sub, data = gen_par_alpha_learn,mean)
...
for (s in 1:Nsub) {
  ...
  ldc_file_bin <- paste0('both_model/ldcfit_',subs[s],'.Rdata')
  if (file.exists(ldc_file_bin)) {
    load(ldc_file_bin)
  ...
  ldc_file_bin_mid <- paste0('both_model_bin_mid/ldcfit_',subs[s],'.Rdata')
  if (file.exists(ldc_file_bin_mid)) {
    load(ldc_file_bin_mid)
...
ggplot(par, aes(x = eta_a, colour = model, label = model)) +
  geom_density() +
  theme_bw()
ggplot(par, aes(x = eta_b, colour = model, label = model)) +
  geom_density() +
  theme_bw()
