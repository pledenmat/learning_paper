...
#' - Check fitting parameters are as intended
#'    - binning, Nupdate, Nerr, error function, target
#' - Check that you are retrieving correct model fits 
...
for (subject in 1:length(subs)) {
  ...
  params=ldc.results$optim$bestmem[5:7]
  #' Initialize weights
  w <- params[1:3]
