...
cross_entropy <- function(y,y_pred){
  return(- sum( (1 - y)*log(1 - y_pred) + y*log(y_pred)))
}
grad <- function(x,y,y_pred){
  return((y - pred_j)*x[,c(1,2)]/sqrt(x[,3]))
}
train_model <- function(x,w,y,eta=.1,momentum=0,
                        M = 100,th=0.0000001,verbose=F,trace=F,model="nolinear"){
  ...
  results <- list(w=w,acc=acc,param_iter=dat_iter,niter=i*ntrain)
  return(results)
}
...
M <- 1
eta <- .1
results <- train_model(x,w,y,eta=eta,M=M,trace=T,verbose=T)
save(results,file="train_trace_loweta.Rdata")
...
cross_entropy(y,model_nolinear(x,c(alpha,beta,1)))
plot(results_higheta$param_iter$alpha[1:100])
plot(results_higheta$param_iter$beta)
plot(results_higheta$param_iter$err)
...
n_train <- c(5,10,20,50,100,200,dim(Data)[1])
n_rep <- 10
...
for (a in alpha_grid) {
  for (b in beta_grid) {
    w <- c(a,b,1)
    y_pred <- model_nolinear(x,w)
    grid[grid$alpha==a&grid$beta==b,"ce"] <- sum((y_pred - y)^2)/length(y) 
