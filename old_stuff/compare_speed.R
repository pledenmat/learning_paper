...
par2 <- data.frame(eta = NA, a0 = NA, b0 = NA, cost_ldc = NA,
                  ...
                  difflevel = rep(difflevels, length.out = totlen),
                  manip=NA)
parfast <- data.frame(eta = NA, a0 = NA, b0 = NA, cost_ldc = NA,
                  sub = rep(subs, each = length(conditions)*length(difflevels)),
                  condition = rep(conditions,each = length(difflevels),
                                  length.out=totlen),
                  difflevel = rep(difflevels, length.out = totlen),
                  manip=NA)
parfast_shuffle <- data.frame(eta = NA, a0 = NA, b0 = NA, cost_ldc = NA,
                      sub = rep(subs, each = length(conditions)*length(difflevels)),
                      condition = rep(conditions,each = length(difflevels),
                                      length.out=totlen),
                      difflevel = rep(difflevels, length.out = totlen),
                      manip=NA)
parskip <- data.frame(eta = NA, a0 = NA, b0 = NA, cost_ldc = NA,
                      sub = rep(subs, each = length(conditions)*length(difflevels)),
                      condition = rep(conditions,each = length(difflevels),
                                      length.out=totlen),
                      difflevel = rep(difflevels, length.out = totlen),
...
for (s in subs) {
  ...
  ldc_file <- paste0('w0/batch_',Nupdate_per_trial,'_1/ldcfit_',s,'.Rdata')
  if (file.exists(ldc_file)) {
    load(ldc_file)
  }else{
    print(paste("Failed to retrieve participant",s))
  ...
  ldc_file <- paste0('w0/batch_',Nupdate_per_trial,'_fast/ldcfit_',s,'.Rdata')
  if (file.exists(ldc_file)) {
    load(ldc_file)
  }else{
    print(paste("Failed to retrieve participant",s))
  }
  parfast[parfast$sub==s,"a0"] <- ldc.results$optim$bestmem[1]
  parfast[parfast$sub==s,"b0"] <- ldc.results$optim$bestmem[2]
  parfast[parfast$sub==s,"eta"] <- ldc.results$optim$bestmem[4]
  parfast[parfast$sub==s,"cost_ldc"] <- ldc.results$optim$bestval
  parfast[parfast$sub==s,"manip"] <- unique(temp_dat$manip)
  
  ldc_file <- paste0('w0/batch_',Nupdate_per_trial,'_fast_shuffle/ldcfit_',s,'.Rdata')
  if (file.exists(ldc_file)) {
    load(ldc_file)
  }else{
    print(paste("Failed to retrieve participant",s))
  }
  parfast_shuffle[parfast_shuffle$sub==s,"a0"] <- ldc.results$optim$bestmem[1]
  parfast_shuffle[parfast_shuffle$sub==s,"b0"] <- ldc.results$optim$bestmem[2]
  parfast_shuffle[parfast_shuffle$sub==s,"eta"] <- ldc.results$optim$bestmem[4]
  parfast_shuffle[parfast_shuffle$sub==s,"cost_ldc"] <- ldc.results$optim$bestval
  parfast_shuffle[parfast_shuffle$sub==s,"manip"] <- unique(temp_dat$manip)

  ldc_file <- paste0('w0/batch_',Nupdate_per_trial,'_skip/ldcfit_',s,'.Rdata')
  if (file.exists(ldc_file)) {
    load(ldc_file)
  }else{
    print(paste("Failed to retrieve participant",s))
  }
  parskip[parskip$sub==s,"a0"] <- ldc.results$optim$bestmem[1]
  parskip[parskip$sub==s,"b0"] <- ldc.results$optim$bestmem[2]
  parskip[parskip$sub==s,"eta"] <- ldc.results$optim$bestmem[4]
  parskip[parskip$sub==s,"cost_ldc"] <- ldc.results$optim$bestval
  parskip[parskip$sub==s,"manip"] <- unique(temp_dat$manip)
...
parfast$fit <- "fast"
parfast_shuffle$fit <- "fast_shuffle"
parskip$fit <- "skip"
par$fit <- "slow"
par2$fit <- "slow2"

par_tot <- rbind(par,par2,parfast,parskip,parfast_shuffle)
